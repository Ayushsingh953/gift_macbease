...
import { View, Text, Image, StyleSheet, FlatList, Dimensions, TouchableOpacity, Pressable } from "react-native";
...
export default function ImageList({ navigation, route }) {
    ...
    return (
        <>
            <SharedElement id={`item.${activeItem.key}.image1`}>
                <SharedElement id={`item.${activeItem.key}.image2`}>
                    <Image
                        source={activeItem.image}
                        style={{ width: "100%", height: "100%", shadowColor: "rgba(0,0,0,0.8)", shadowRadius: 22, shadowOffset: { width: 12, height: 12 }, shadowOpacity: 0.4 }}
                        resizeMode="cover"
                        blurRadius={5}
                    />
                </SharedElement>
            </SharedElement>
            <SharedElement id={`item.${activeItem.key}.text1`} style={{ position: "absolute", zIndex: 99, top: 15 }}>
                <SharedElement id={`item.${activeItem.key}.text2`} style={{ position: "absolute", zIndex: 99, top: 15 }}>
                    <Animatable.Text
                        animation={animation}
                        delay={1000}
                        style={styles.text}
            ...
            <Animatable.View
                ...
                delay={3000}
            >
                <Pressable style={({ pressed }) => pressed && [{ opacity: 0.5 }]} onPress={() => { navigation.push("composeMsg") }}>
                    <Text style={{ fontWeight: "bold", color: "white", fontSize: 16 }}>Let's go!</Text>
                </Pressable>
            </Animatable.View>
            <FlatList
                data={getRightData()}
